...
def genere_X_t(i, means, std):
    """
    Génère une réalisation X_t issue de la loi N(means[i], std[i])
...
def UCB_delta(n, delta, k, means, std):
    ...
    for t in range(n):
        all_A_t = np.zeros(k)
        for i in range(k):
            all_A_t[i] = UCB_i(i, delta, mu, T) ##On calcul UCB_i pour tous les bras
        A[t] = np.argmax(all_A_t) ##On prend le bras qui a le UCB_i le plus haut
        X[t] = genere_X_t(A[t], means, std) ##On génère X_t la réalisation à partir du bras choisi
